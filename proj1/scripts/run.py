...
pid = os.fork()
if pid == 0:
	subprocess.Popen("rmiregistry", cwd="../src/build")
	exit(0)
...
for i in range(npeers):
	pid = os.fork()
	if pid == 0:
		subprocess.call(["./peer.sh", str(version), str(acessPoint + i), str(acessPoint + i), "230.0.0.1", str(port), "230.0.0.2", str(port), "230.0.0.3", str(port)])
		exit(0)
